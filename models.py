...
class UserInfo(db.Expando):
  ...
  last_checkin_str = db.StringProperty() 
  #last_checkin = db.IntegerProperty() #TODO changes need to be made at some point to use the since instead of sinceid. by that time all users should have a datetime, but we can hadnle the case in which they don't somehow?
